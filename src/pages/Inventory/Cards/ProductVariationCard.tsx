...
function ProductVariationCard({ variation }: Props) {
  ...
  const [editColor, setEditColor] = useState(variation.color ?? "");
  const [editSize, setEditSize] = useState(variation.size ?? "");
  const [editStock, setEditStock] = useState(variation.stock);
  const [saving, setSaving] = useState(false);

  function openEdit() {
    setEditColor(variation.color ?? "");
    setEditSize(variation.size ?? "");
    setEditStock(stock);
    setEditOpen(true);
  }
  ...
  async function handleSave() {
    setSaving(true);
    try {
      const updated = await ProductService.updateVariation(variation.id, {
  ...
  async function handleIncrement(e: React.MouseEvent) {
    ...
    setLoading("inc");
    try {
      const updated = await ProductService.addStock(variation.id);
      setStock(updated.stock);
      toast.success(`Estoque: ${updated.stock} un.`, { position: "bottom-center", autoClose: 2500 });
    } catch (error: any) {
      const status = error.response?.status;
      if (status && status < 500 && ![401, 403, 404].includes(status)) {
  ...
  async function handleDecrement(e: React.MouseEvent) {
    ...
    setLoading("dec");
    try {
      const updated = await ProductService.removeStock(variation.id);
      setStock(updated.stock);
      toast.success(`Estoque: ${updated.stock} un.`, { position: "bottom-center", autoClose: 2500 });
    } catch (error: any) {
      const status = error.response?.status;
  ...
  return (
    <>
      ...
      <div className="rounded-2xl bg-white border border-gray-200 shadow-sm hover:shadow-md hover:border-indigo-300 transition-all overflow-hidden">
        ...
        {/* Zona de estoque — botões +/- */}
        <div className="px-4 py-2.5 flex items-center justify-between">
          <span className="text-xs text-gray-400 font-medium tracking-wide">Estoque</span>
